...
<table>
    ...
    //连接数据库
    $conn=connnetDb();
    $result=mysqli_query($conn,"SELECT * FROM table1 where style=2");
    if (!Empty($_POST['username'])){
        $username=$_POST['username'];
        $result=mysqli_query($conn,"SELECT * FROM table1 WHERE username LIKE '%$username%' and style=2");
    }else{
        $result=mysqli_query($conn,"SELECT * FROM table1 where style=2");
    }
    //获取数据表的数据条数
